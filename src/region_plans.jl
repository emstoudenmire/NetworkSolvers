...
import NamedGraphs.GraphsExtensions: default_root_vertex, post_order_dfs_vertices
...
function basic_region_plan(
  ...
  fwd_sweep = [(collect(region), sweep_kwargs) for region in partition(vertices, nsites, 1)]
